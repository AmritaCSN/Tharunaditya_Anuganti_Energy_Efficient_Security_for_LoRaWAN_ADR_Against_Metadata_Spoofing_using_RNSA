...
void PacketForwarder::initialize(int stage)
{
    if (stage == 0) {
        ...
        rssiOffset      = par("rssiOffset");
        snrOffset       = par("snrOffset");
        snrOffsetMode   = par("snrOffsetMode").stdstringValue();
        snrOffsetMin    = par("snrOffsetMin");
        snrOffsetMax    = par("snrOffsetMax");
        snrOffsetStep   = par("snrOffsetStep");
        snrOffsetPeriod = par("snrOffsetPeriod");
...
void PacketForwarder::processLoraMACPacket(Packet *pk)
{
    ...
    // Byzantine attack: corrupt RSSI/SNR before forwarding to the network server
    if (isMalicious) {
        // Compute effective SNR offset based on attack mode
        double effectiveSnrOffset = snrOffset; // default: "fixed" mode

        if (snrOffsetMode == "step") {
            // Ramp: starts at snrOffsetMin, increases by snrOffsetStep every snrOffsetPeriod packets
            int stepIdx = maliciousPktCount / snrOffsetPeriod;
            effectiveSnrOffset = snrOffsetMin + stepIdx * snrOffsetStep;
            if (effectiveSnrOffset > snrOffsetMax) effectiveSnrOffset = snrOffsetMax;
        } else if (snrOffsetMode == "random") {
            // Uniform random between snrOffsetMin and snrOffsetMax each packet
            double range = snrOffsetMax - snrOffsetMin;
            effectiveSnrOffset = snrOffsetMin + (range > 0 ? uniform(0, range) : 0.0);
        }
        // "fixed" mode: use snrOffset as-is (no change needed)

        maliciousPktCount++;
        frame->setRSSI(frame->getRSSI() + rssiOffset);
